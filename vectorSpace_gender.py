...
class VectorSpace_gender:
    ...
    attrWithMultipleCategoricalValues = [4, 5, 6, 12] #numerical vector space has one feature for each categorical value
    attrWithNumericalValues = [0, 1, 2, 3]  #numerical feature has the same value as the catigorical one
    ...
    allGenderValues = ["", "", ["M/F", "N"], ["M", "F", "N"]]
    genderValues = allGenderValues[numOfGenders]
    ...
    def numericalVectorSpace(self, filenames, gender=numOfGenders):
        s = VectorSpace_gender #This class
            ...
            for c, a in zip(row, s.attrForVectorSpace):
                ...
                if a not in s.attrWithMultipleCategoricalValues:
                    if a in s.attrWithNumericalValues:  #these features take the same values as categorical ones
                        ...
                        if categoricalFeature_names[a] == "GENDER" and gender == 2:
                            if c.replace(" ", "") == "N":
                                numerical_row[index] = "N"
                            if c.replace(" ", "") == "M" or c.replace(" ", "") == "F":
                                numerical_row[index] = "M/F"
                        else:
